...
function ToggleRow({ label, checked, onChange }) {
 return (
  <label className="flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-cozy-borderpx-3 py-3 shadow-sm bg-cozy-surface">
   ...
   <input
    ...
    className="h-5 w-5 shrink-0 rounded border-cozy-border text-cozy-primary focus:ring-2 focus:ring-cozy-primary border-cozy-border bg-cozy-bg"
   />
  </label>
 );
}
...
export default function GameSummary({ summary, onLeave, readOnlyRecap }) {
 ...
 return (
  <div className="flex min-h-full flex-coltext-cozy-text bg-cozy-bg">
   <header className="shrink-0 border-b border-cozy-border px-4 py-4 border-cozy-border">
    <div className="mx-auto flex max-w-3xl flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
     <div>
      ...
      <p className="font-mono text-lg font-semibold text-cozy-primary">
       {summary.code}
      </p>
     ...
     <div className="flex flex-wrap gap-2">
      <button
       ...
       className="rounded-lg bg-cozy-primary px-4 py-2.5 text-sm font-semibold text-cozy-bg disabled:opacity-50"
      >
       {shareBusy ? "…" : "Partager"}
      </button>
      <button
       type="button"
       onClick={toggleTheme}
       className="rounded-lg border border-cozy-borderpx-4 py-2.5 text-sm font-medium bg-cozy-surface"
      >
    ...
    <div className="mx-auto mt-4 max-w-3xl rounded-xlp-4 shadow-sm ring-1 ring-cozy-border">
     <div className="flex flex-wrap items-center gap-2">
      <button
       type="button"
       onClick={() => setPlaying((p) => !p)}
       className="rounded-lg bg-cozy-primary px-4 py-2 text-sm font-bold text-cozy-bg"
      >
      ...
      <button
       ...
       className="rounded-lgpx-3 py-2 text-sm font-semibold bg-cozy-surface"
      >
       Début
      </button>
      <span className="text-xs text-cozy-text-muted">Vitesse ×{speed}</span>
      <input
       type="range"
       min={2}
       max={40}
       ...
       className="h-2 w-24 accent-cozy-primary"
      />
     </div>
     <input
      type="range"
      ...
      className="mt-3 w-full accent-cozy-primary"
     />
     <div className="mt-1 flex justify-between font-mono text-xs text-cozy-text-muted">
   ...
   <div className="flex shrink-0 flex-wrap gap-2 border-b border-cozy-border px-4 py-2 border-cozy-border">
     ...
     <button
      ...
      className={`rounded-lg px-3 py-1.5 text-xs font-medium ${
       basemapId === id
        ...
        : "bg-cozy-surface text-cozy-text"
      }`}
     >
      {b.name}
     </button>
   ...
   <div className="mx-auto grid w-full max-w-3xl shrink-0 grid-cols-1 gap-3 p-3 md:grid-cols-2">
    <div className="space-y-2">
     <p className="text-xs font-semibold uppercase text-cozy-text-muted">
      Joueurs
     </p>
     {players.map((p) => (
      <label
       key={p.sessionId}
       className="flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-cozy-borderpx-3 py-3 shadow-sm bg-cozy-surface"
      >
       <span className="flex min-w-0 items-center gap-2">
        ...
        />
        <span className="truncate text-sm font-medium text-cozy-text">
         {p.nickname}
        </span>
       </span>
       <input
   ...
   <div className="h-[42vh] min-h-[220px] w-full shrink-0 border-b border-cozy-border">
    <MapContainer
     center={center}
     zoom={15}
   ...
   <div className="min-h-0 flex-1 overflow-auto px-4 py-3">
    <h2 className="mb-2 text-xs font-semibold uppercase text-cozy-text-muted">
     Chronologie
    </h2>
    ...
    {(summary?.partyChat || []).length > 0 && (
     <>
      <h2 className="mb-2 mt-6 text-xs font-semibold uppercase text-cozy-text-muted">
      ...
      <ul className="max-w-3xl space-y-2">
       {[...(summary.partyChat || [])]
        ...
        .map((m) => (
         <li
          key={m.id}
          className="rounded-lg border border-cozy-border bg-cozy-bg/80 p-2 text-sm bg-cozy-surface/80"
         >
          <p className="text-[10px] font-semibold text-cozy-text-muted">
           {formatClock(m.t)} · {m.nickname}
          </p>
          {m.type === "text" && (
           <p className="mt-0.5 whitespace-pre-wrap break-words text-cozy-text">
            {m.text}
           </p>
          )}
          {m.type === "location" && m.lat != null && m.lng != null && (
           <p className="mt-0.5 text-xs text-cozy-text-secondary">
          ...
          {m.type === "image" && m.image && (
           <div className="mt-1">
            <img
             src={m.image}
             alt=""
             className="max-h-36 max-w-full rounded-md border border-cozy-border"
            />
            {m.lat != null && m.lng != null && (
             <p className="mt-1 text-[10px] text-cozy-text-muted">
              📍 {Number(m.lat).toFixed(5)}, {Number(m.lng).toFixed(5)}
             </p>
            )}
           </div>
   ...
   <div className="shrink-0 border-t border-cozy-border p-4 border-cozy-border">
    <button
     type="button"
     onClick={onLeave}
     className="mx-auto block w-full max-w-md rounded-lg bg-cozy-primary py-3.5 text-base font-semibold text-cozy-bg"
    >
     Retour
    </button>
   </div>
   ...
   {shareOpen && (
    <div
     className="fixed inset-0 z-[12000] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm"
     role="dialog"
     aria-modal="true"
    >
     <div className="w-full max-w-md rounded-xlp-6 shadow-2xl ring-1 ring-cozy-border">
      <h3 className="text-lg font-bold text-cozy-text">
       Partager le récap
      </h3>
      ...
      {publicRecapUrl ? (
       <div className="mt-5 flex flex-col items-center gap-4 sm:flex-row sm:items-start">
        <div className="rounded-lgp-2 ring-2 ring-cozy-primary/20">
         <QRCodeSVG value={publicRecapUrl} size={160} level="M" />
        </div>
        <div className="min-w-0 flex-1">
         <p className="break-all font-mono text-xs text-cozy-primary">
          {publicRecapUrl}
         </p>
         <button
          type="button"
          onClick={copyRecap}
          className="mt-3 w-full rounded-lg bg-cozy-primary py-2.5 text-sm font-semibold text-cozy-bg sm:w-auto sm:px-5"
         >
       ...
       <p className="mt-4 text-sm text-cozy-yellow">
      ...
      <button
       ...
       className="mt-6 w-full rounded-lg border border-cozy-border py-3 text-sm font-semibold"
      >
       Fermer
      </button>
     </div>
