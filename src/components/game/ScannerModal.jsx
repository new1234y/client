    ...
    >
      <div className="flex shrink-0 items-center justify-between pb-2">
        ...
        <button
          type="button"
          onClick={onClose}
          className="rounded-[8px] bg-slate-800 px-4 py-2 text-sm font-medium text-white"
        >
          Fermer
        </button>
      </div>

      <div className="shrink-0 rounded-[8px] border border-white/10 bg-white/5 p-4 text-sm leading-relaxed text-slate-200">
        <p className="font-semibold text-white">Votre rôle : chat (traqueur)</p>
      ...
      {err && (
        <p className="mb-2 mt-3 rounded-[8px] bg-red-950/80 p-3 text-sm text-red-200">{err}</p>
      )}

      <div
        id={regionId}
        className="mx-auto mt-3 w-full max-w-md flex-1 overflow-hidden rounded-[8px] bg-black ring-1 ring-white/10"
      />
    </div>
  );
}
